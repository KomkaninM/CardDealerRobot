...
class IRReaderNode(Node):
    ...
    def tick(self):
        ...
        # Publish detection status only on change
        if detected != self.last_state:
            ...
            # Rising edge (False -> True): start a new window
            if (prev is False or prev is None) and detected is True:
                self.reset_window_accumulator()
                # optional: also take an immediate first sample if /odom already exists
                self.accumulate_yaw()
                self.get_logger().info("Start accumulating yaw for detected window.")

            # Falling edge (True -> False): compute mean and publish once
            if prev is True and detected is False:
                ...
                if mean_deg is None:
                    ...
                    if self.also_publish_lcd:
                        lcd2 = String()
                        lcd2.data = f"CENTER ANGLE|{ang.data:.1f} deg"
                        self.pub_lcd.publish(lcd2)
